...
Rules 是不可随意绕过的工程约束。它们用于保证流程有纪律、有证据、有审计链路。

当前已有 6 类规则：

| 规则 | 作用 |
| --- | --- |
| `engineering-governance` | 保证需求、规格、开发、审查、验收按顺序执行 |
| `artifact-traceability` | 保证所有产物可以通过 `change-id` 串起来 |
| `openspec-rules` | 规定 OpenSpec 是规格和验收合同 |
| `tdd-rules` | 规定默认使用 TDD，并记录测试证据 |
| `review-rules` | 规定代码审查输出结构化问题清单 |
| `acceptance-rules` | 规定人工批准和证据完整后才能归档 |

Rules 的价值是避免 AI agent “自作主张”。例如：
...
| Acceptance Officer | 检查证据并完成验收归档 |
...
拒绝条件很关键。它让 agent 不会越权。例如 Fullstack Developer 在没有 OpenSpec change 时应拒绝实现，Acceptance Officer 在没有人工审批时应拒绝归档。
...
Acceptance Officer 检查所有证据完整后，归档 OpenSpec change，并生成验收报告。
...
它保留了人工审查和风险接受机制，适合企业研发流程，而不是完全自动化的个人脚本。
